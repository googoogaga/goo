...
several limitations in the current implementation.  It
represents a rudimentary but usable subset of typical thread
operations.
...
\method{thread-join}{t|<thread>}
  \doc{Causes current thread to wait for the termination of \code{t}.}
...
\method{enqueue!}{pipe|<pipe> x => <pipe>}
  \doc{Adds \code{x} to \code{pipe}.} 
\method{dequeue!}{pipe|<pipe> => <any>}
...
\kode{doc}, \kode{bin}, \kode{lib}, \kode{mods}, and \kode{emacs}.  You
can just run \goo\ from the \kode{bin} subdirectory.

In the case of a development tarball, you must install it.
After unpacking, there will be five directories: \kode{doc},
\kode{bin}, \kode{c}, \kode{src}, and \kode{emacs}.  Edit the
\kode{Make.inc} file in the top level directory, to configure the
installation directory root using the \kode{PREFIX} if \kode{/usr} is
inapproriate.  Executing \kode{make install} will rebuild \goo\ with
the proper roots, creating \kode{lib} and \kode{mods} directories, and
installing \goo\ in \kode{bin} and setting up \kode{doc}.  For the
simplest installation, just set \kode{GOO\_ROOT} to your desired
installation location and type \kode{make simple-install}.  On
windows, it is necessary to add \kode{GOO\_BIN} to your \kode{PATH}.
...
% \end{exv}
% On linux of course you would use forward slashes and 
