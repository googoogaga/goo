...
\T\addtolength{\evensidemargin}{-0.5in}
\T\addtolength{\oddsidemargin}{-0.5in}
...
Vertical bars are tokenized immediately and separately and have
...
bindings shadow bindings of the same name.
...
name as the file.  Modules can import bindings exported by other
modules.  

\begin{defs}
\form{DV}{(DV ,var ,form)}
  \doc{defines a variable named \kode{(var-name ,var)} with an initial value
  \kode{,form} (cf. Scheme's \kode{DEFINE}).}
\form{DEF}{(DEF ,var ,val)}
  \doc{binds \kode{,var} to \kode{,val} and evaluates remainder of current
       body in the context of that binding.}
...
\form{REP}{(REP ,name ((,var ,init) ...) \snl ,@body) }
...
\method{add-prop}{owner getter|<gen> setter|<gen> \snl type|<type> init|<fun>} % TODO: T?
  ...
  the argument.}
\form{DP}{(DP ,name (,oname|,owner => ,type) \snl [,@init])}
...
\form{OP}{(OP ,op-arg ...)}
  ...
  arguments, where \kode{,op-arg} is either an implicit parameter
  \kode{\_} or \kode{...} or an s-expression potentially containing
  further op-args.  The required parameters \kode{\_}'s are found
...
\begin{ex}
(MIF (,a ,b) '(1 2) (lst a b)) \snl $\rightarrow$ (1 2)
\end{ex}
  and
\begin{ex}
(MIF (,a ,@b) '(1 2) (lst a b)) \snl $\rightarrow$ (1 (2))
...
\generic{num-to-str}{x|<num> => <str>}
  \doc{$equiv$ \kode{(num-to-str x 10)}}
...
keys~\footnote{Currently value modification has not be completely
teased out.}.  The hope is that functional (nondestructive)
programs will be both more succinct, understandable, and efficient
than an equivalent destructive programs.  Only core collection
operators are given destructive versions.  All others can be built out
of nondestructive operators followed by \kode{into}.
...
an enumeration class and the enumeration protocol: \kode{enum},
...
\begin{defs}
\syntax{,for-clause}{(,var ,col) \snl | ((tup ,keyvar ,var) ,col)}
...
\generic{packer}{init add|<fun> res|<fun>}
  \doc{returns a simple packer with that starts its value out
...
\generic{tab-gc-state}{x|<tab> => <any>}
  \doc{GC specific.}
\generic{id-hash}{x|<tab> => (tup hash gc-state)}
...
\generic{find}{x|<seq> y|<seq> => (t?\ <int>)}
...
\generic{sub}{x|<seq> from|<int> below|<int> \snl => <seq>}
...
\generic{sub-setter}{dst|<seq!> src|<seq!> \snl from|<int> below|<int>}
...
\generic{buf}{elts|... => <sec>}
  \doc{returns new buf with elements \kode{elts}.}
\class{<str>}{<flat> <mag>}
  \doc{\goo\ currently implements ASCII strings.}
\generic{str}{elts|... => <str>}
  \doc{returns new string with elements \kode{elts}.}
\generic{case-insensitive-string-hash}{x|<tab> \snl => (tup hash|<any> gc-state|<any>)}
...
\generic{range}{from|<num> test|<fun> lim|<fun> => <range>}
  \doc{$\equiv$ \kode{(range-by from test lim 1+)}}
...
\generic{first-then}{first|<any> then|<any> => <step>}
  \doc{returns a new step function.}
\class{<cycle>}{<seq>}
...
\generic{default-handler-description}{c|<condition> => <str>}
  ...
  condition.}
\generic{build-condition-interactively}{cond-type|<condition> in out
           \snl => <condition>}
...
\class{<simple-condition>}{<condition>}
  \doc{a condition consisting of a format message and arguments.}
\property{condition-message}{x|<simple-condition> => <str>}
  \doc{returns format string.}
\property{condition-arguments}{x|<simple-condition> => <lst>}
  \doc{returns format string arguments.}
...
\class{<simple-error>}{<error> <simple-condition>}
  \doc{an error that consists of a format message and arguments.}
...
\syntax{,try-options}{,condition-type-name \snl | ,try-option-list}
...
\generic{format}{x|<out-port> message|<seq> args|...}
...
\begin{defs}
\generic{load}{filename|<str> modname|<sym> \snl => <any>}
...
\begin{exv}
(autoload 'goo-mode "goo" 
  "Major mode for editing Goo source." t)
...
\begin{packeditemize}
\item You can add ``font-lock'' mode by adding the following to your
\kode{.emacs}:
\begin{exv}
(global-font-lock-mode t)
\end{exv}
\item In a given buffer, you can toggle font-lock with \kode{M-x font-lock-mode}
\item Check out the ``Index'' menu item in a goo buffer.
\end{packeditemize}

Load \kode{EMACS/goo-font-lock.el} for a color coded parenthesis
nesting aid~\footnote{The original idea was dreamed up and first
implemented by Andrew Sutherland and then improved by James Knight}.   
...
\begin{exv}
(autoload 'run-goo  "goo-shell" 
          "Run an inferior Goo process." t)
(setq auto-mode-alist
      (cons '("\\.goo\\'" . goo-mode) 
             auto-mode-alist))
(setq goo-program-name 
      "/home/ai/jrb/goo/goo") 
\end{exv}
