...
\title{{\Huge \goo\ Reference Manual v32 } % \\
...
lisp-syntaxed Dylan~\cite{Shalit:1996} and an object-oriented 
...
\generic{col-res-type}{t|<col> x|<col> => <col>}
  \doc{appropriate instantiable type for creating collection results,
       where the default is their own type.}
...
operators followed by \kode{into}.  The \kode{!} suffixed functions
...
appropriate installation area.  There are four directories:
\kode{doc}, \kode{bin}, \kode{src}, and \kode{emacs}.  Edit the
\kode{Makefile} in the top level directory, to configure the
installation directory root using the \kode{PREFIX} if \kode{/usr}
is inapproriate.  Executing \kode{make} will rebuild \goo\ with the
proper roots, creating \kode{lib} and \kode{mods} directories, and
installing \goo\ in \kode{bin} and setting up \kode{doc}.
