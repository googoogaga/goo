...

#ifndef emacs
#define fatal(args...) fprintf (stderr, ##args ), exit (1)
#else
#include <config.h>
...
#define MDEBUGADJUST(__ct,__fileaddr)		\
  ...
  msync(old_base, stat_buf.st_size, MS_SYNC | MS_INVALIDATE);
  msync(new_base, new_file_size, MS_SYNC | MS_INVALIDATE);

  if(munmap (old_base, stat_buf.st_size))
    fatal ("Can't munmap (%s): errno %d\n", old_name, errno);

  if (close (old_file))
    fatal ("Can't close (%s): errno %d\n", old_name, errno);

  if (munmap(new_base, new_file_size))
    fatal ("Can't munmap (%s): errno %d\n", new_name, errno);
